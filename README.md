...
如果有一些视频在下载时未能进行 AI 分析（例如当时未开启 AI 功能），可以使用此脚本进行补救。
...
**环境变量配置**：
- `ANALYZE_CONCURRENCY`: 并发处理数量（默认 3），可根据机器性能调整，提高处理速度
- `ANALYZE_BATCH_SIZE`: 每次处理的批次大小（默认 5）
- `ANALYZE_INTERVAL`: 检查间隔（默认 1小时）
- `ANALYZE_UI_PORT`: 分析控制台端口（默认 3002）

启动脚本后，访问 `http://localhost:3002` 即可查看实时的 **AI 分析控制台**，提供以下功能：
- 实时监控分析进度和速度
- 查看并发任务状态
- 实时日志流
- 详细的统计数据（成功率、媒体类型分布等）
